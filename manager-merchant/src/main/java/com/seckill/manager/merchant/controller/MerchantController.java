...
public class MerchantController {
    ...
    public SeckillResult registerUser(@RequestBody ManagerMerchant merchantInfo) {
        ...
        try {
            ...
            return new SeckillResult(SeckillCodeMapping.SYSTEM_ERROR, "商户入驻失败", e);
        }
    ...
    public SeckillResult login(@RequestBody ManagerMerchant merchantInfo) {
        if (StringUtils.isEmpty(merchantInfo.getAccount())){
            return new SeckillResult(SeckillCodeMapping.PARAMETER_ERROR, "商家用户名不能为空");
        }
        ...
        try {
            ...
            if (merchant != null) {
                ...
                if (!result) {
                    return new SeckillResult(SeckillCodeMapping.BUSINESS_FAIL, "用户名或者密码不对");
                }
            } else {
                return new SeckillResult(SeckillCodeMapping.BUSINESS_FAIL, "用户名或者密码不对");
            }
            ...
            return new SeckillResult(SeckillCodeMapping.SYSTEM_ERROR, "登录错误", e);
        }
