...
public class ShopController {
    ...
    public SeckillResult applyShop(@RequestBody ManagerShop shopInfo){
        if (StringUtils.isEmpty(shopInfo.getShopName())) {
            return new SeckillResult(SeckillReturnCodeMapping.PARAMETER_ERROR, "店铺名称不能为空");
        }
        if (StringUtils.isEmpty(shopInfo.getShopBussinessScope())) {
            return new SeckillResult(SeckillReturnCodeMapping.PARAMETER_ERROR, "店铺经营范围不能为空");
        }
        if (StringUtils.isEmpty(shopInfo.getBusinessLicense())) {
            return new SeckillResult(SeckillReturnCodeMapping.PARAMETER_ERROR, "店铺营业执照不能为空");
        }
        try {
            return new SeckillResult(shopService.addShopInfo(shopInfo));
        } catch (Exception e) {
            logger.error("添加商店信息错误，原因{}", e);
            return new SeckillResult(SeckillReturnCodeMapping.SYSTEM_ERROR, "添加商店信息错误", e);
        }
    }
    ...
    public SeckillResult searchShop(@RequestBody ManagerShop shopInfo){
        ...
        if (shopInfo.getState() == null) {
            return new SeckillResult(SeckillReturnCodeMapping.PARAMETER_ERROR, "店铺状态不能为空");
        }
